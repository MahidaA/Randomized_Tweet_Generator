...
DELAY = 3  # Seconds to wait in between processing each set of tweets
...
while tweet_count < MAX_TWEETS:
    ...
    lowest_id = timeline[len(timeline) - 1].id
...
# Storing words as objects allows us to also keep track of what they were
...
# Reads through the text of a tweet and stores each word as a Word object in
# a list of Words.
...
def process_tweet(tweet_text, wlist):
    ...
    for i in range(1, len(simple_words) - 2):
...
def start_chain(wlist, gen_tweet):
    # List of Words that can be used to start a tweet. This draws from all the
    # words the given user used to start their tweets.
    valid_starters = []
...
def continue_chain(wlist, gen_tweet):
    ...
    # If the chosedn word is a tweet ender (not followed by anything),
    # return 0 to indicate it.
...
# Unicode is used because of emojis. Disclaimer: emojis will probably
# appear in the text file as squares or junk characters due to lack of emojis
# in most text editor fonts.
